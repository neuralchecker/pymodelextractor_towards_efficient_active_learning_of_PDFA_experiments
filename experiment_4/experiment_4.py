                ...
                if i > 0:
                    if algorithm_name == 'QuantNaryTreeLearner':
                        ...
                        if result.info['observation_tree'] is None:
                            ...
                            tree_depth = result.info['observation_tree'].root.depth
                    else:
                    ...
                    log_probability_error, wer,ndcg, out_of_partition, out_of_tolerance, absolute_error_avg  = compute_stats(pdfa, extracted_model)
                    results.append((algorithm_name, pdfa.name, len(pdfa.weighted_states), i, secs, result.info['last_token_weight_queries_count'], result.info['equivalence_queries_count'], ot_pref, ot_suff, tree_depth, log_probability_error, wer,ndcg, out_of_partition, out_of_tolerance))
    pbar.close() 
    dfresults = pd.DataFrame(results, columns = ['Algorithm', 'Instance', 'Number of States' ,'RunNumber','Time (s)','LastTokenQuery', 'EquivalenceQuery', 'OT Prefixes', 'OT Suffixes', 'Tree Depth', 'LogProbError','WER','NDCG','OOPartition','OOTolerance']) 
    dfresults.to_csv('./experiment_4/results/results_'+datetime.datetime.now().strftime("%d_%m_%Y_%H_%M_%S")+'.csv') 
